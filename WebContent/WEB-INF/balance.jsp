...
<body>
    ...
    <div class="main">
        <div class="balance-container">
            <div class="form-head">
                <h2 style="margin: 15px 0;">Add Balance</h2>
            </div>
            <form action="balance.do" class="booking" method="POST">
                <div class="form-in">
                    <div class="labels2">
                    <input type="hidden" name="csrfToken" value="${csrfToken}" />
	            	<c:forEach var="field" items="${form.visibleFields}">
							<label style="color: red;">${field.error}</label>
				  			<input type="${field.type}" class="form-input2" placeholder="${field.label }" id="${field.name}" name="${field.name}" value="${field.value}" required><br><br>
					</c:forEach>
                    </div>
                </div>
                <input type="submit" value="Add Balance" name = "action" class="sbt">
            </form>
        </div>
    </div>
    <footer>
